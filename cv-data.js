window.CV_DATA = {
  "meta": {
    "nameCn": "何浚翥",
    "nameEn": "He Junzhu",
    "eyebrow": "Urban · Health · Cultural Economics",
    "intro": "城市经济学在读，研究兴趣聚焦城市经济学、健康经济学和文化经济学。",
    "email": "hejunzhu777@163.com",
    "phone": "+86 188-4558-9327",
    "phoneLink": "+8618845589327"
  },
  "profile": {
    "headline": "在城市、健康与文化之间寻找证据",
    "summary": "研究兴趣聚焦城市经济学、健康经济学和文化经济学。既关注城市蔓延、低碳转型和新型城镇化等宏观空间议题，也通过 CFPS、上市公司与区县面板数据观察家庭、企业和地方政策的微观响应。",
    "focus": [
      "城市经济学：城市空间、经济活力与空间溢出",
      "健康经济学：家庭福祉、教育投资与公共政策",
      "文化经济学：文化交流、意识形态与区域发展",
      "应用微观数据与政策评估"
    ]
  },
  "education": [
    {
      "period": "2024.09 至今",
      "school": "中国人民大学",
      "program": "应用经济学院 & 区域国别研究院 · 城市经济学（在读）",
      "details": "",
      "location": "中国北京"
    },
    {
      "period": "2022.09 - 2024.06",
      "school": "南开大学",
      "program": "经济学院 · 国际商务，经济学硕士（保研）",
      "details": "GPA 90/100",
      "location": "中国天津"
    },
    {
      "period": "2018.08 - 2022.06",
      "school": "哈尔滨工业大学",
      "program": "经济与管理学院 · 国际经济与贸易，经济学 & 文学学士（双学位）",
      "details": "GPA 88.61/100（3.73/4.0）· 综合 Rank 2nd",
      "location": "中国哈尔滨"
    }
  ],
  "exchange": [
    "英国华威大学中英文化交流项目（2021）",
    "台湾东海大学创新管理夏令营优秀学员（2019）",
    "欧洲四国意识形态田野调查（2026）"
  ],
  "research": [
    {
      "role": "第一作者",
      "status": "CSSCI 已发表",
      "statusType": "published",
      "title": "战略性产业政策与企业对外直接投资",
      "journal": "《国际贸易问题》",
      "summary": "围绕“中国制造 2025”战略，使用上市公司数据与双重差分方法，分析产业政策如何影响企业 OFDI 概率与规模。",
      "language": "zh",
      "featured": false
    },
    {
      "role": "通讯作者",
      "status": "SSCI 已发表",
      "statusType": "published",
      "title": "Urban sprawl and economic vitality: Evidence from 284 cities in China",
      "journal": "Journal of Urban Planning and Development",
      "summary": "以 2003-2020 年中国 284 个城市数据评估城市蔓延对经济活力的影响，识别劳动力集聚、人力资本提升与消费扩张等作用路径。",
      "language": "en",
      "featured": true
    },
    {
      "role": "第二作者",
      "status": "CSSCI 已发表",
      "statusType": "published",
      "title": "新型城镇化试点政策与包容性绿色增长",
      "journal": "《经济问题探索》",
      "summary": "将国家新型城镇化综合试点视为准自然实验，基于 2010-2021 年 1819 个区县面板数据讨论政策对包容性绿色增长的影响。",
      "language": "zh",
      "featured": false
    },
    {
      "role": "第一作者",
      "status": "CSSCI 已发表",
      "statusType": "published",
      "title": "最低工资调整对家庭教育投资的影响",
      "journal": "《山西财经大学学报》",
      "summary": "基于 2010-2018 年 CFPS 数据，检验最低工资上调对家庭教育支出的影响，并从就业概率、收入增长与储蓄倾向变化解释其机制。",
      "language": "zh",
      "featured": false
    },
    {
      "role": "第二作者",
      "status": "Under Review",
      "statusType": "review",
      "title": "Rebels and Ruin: Populism's Clash with Green Futures",
      "journal": "Energy Economics",
      "summary": "基于 59 国 1980-2020 年面板数据，结合事件研究法与合成控制法，识别民粹主义对能源转型的多期影响及制度机制。",
      "language": "en",
      "featured": false
    },
    {
      "role": "第一作者",
      "status": "Under Review",
      "statusType": "review",
      "title": "Boomerang of “love”? Son preference and parental health depletion",
      "journal": "Journal of Health Economics",
      "summary": "基于首胎性别准随机性与省际性别比差异，研究儿子偏好如何通过地位竞争损耗父母健康；结果显示，性别比上升会显著恶化首胎女儿父母的身体、心理与主观健康。",
      "language": "en",
      "featured": false
    }
  ],
  "projects": [
    {
      "type": "国家自然科学基金面上项目",
      "title": "双碳目标下数字经济发展对中国城市低碳转型的影响",
      "code": "72473145"
    },
    {
      "type": "国家社科基金重点项目",
      "title": "世界经济新格局下的创新保护研究",
      "code": "18AZD001"
    },
    {
      "type": "国家自然科学基金面上项目",
      "title": "数字经济中的国际直接投资研究",
      "code": "71873071"
    }
  ],
  "conferences": [
    {
      "date": "2025.11",
      "name": "第8届中国制度经济学论坛（北京大学、山东大学）"
    },
    {
      "date": "2023.07",
      "name": "第3届全国大学生发展经济学论文大赛（中国社会科学院大学）"
    },
    {
      "date": "2023.05",
      "name": "首届香樟社会保障论坛（上海财经大学）"
    },
    {
      "date": "2022.09",
      "name": "第5届金融科技创新与风险管理学术会（天津大学、中山大学）"
    }
  ],
  "honors": [
    "全国大学生发展经济学论文大赛一等奖",
    "人民大学一等奖学金",
    "校优秀毕业论文",
    "光华奖学金",
    "南开推免奖学金",
    "校级社会实践一等奖",
    "南开公能奖学金",
    "南开学术专项奖学金",
    "南开大学三好学生",
    "南开大学优秀毕业生",
    "孙运璇海峡交流基金资助"
  ],
  "skills": [
    {
      "title": "语言",
      "body": "中文母语；英语沟通与写作熟练（CET-6 585，IELTS 7.0）。"
    },
    {
      "title": "数据与软件",
      "body": "熟悉 SDC、CSMAR、CNDRS、CFPS、CMDS、CHARLS 等数据库，以及 Python、R、STATA、SPSS 等数据处理软件。"
    },
    {
      "title": "实践",
      "body": "曾参与中国人民大学国家发展与战略研究院、开源证券、京东科技、香樟青苗与达沃斯论坛相关实践。"
    }
  ],
  "contact": {
    "headline": "欢迎交流城市经济学、健康经济学和文化经济学。"
  },
  "admin": {
    "repository": "Olen-hjz777/CV-chinese",
    "branch": "main",
    "dataPath": "cv-data.js"
  }
};
